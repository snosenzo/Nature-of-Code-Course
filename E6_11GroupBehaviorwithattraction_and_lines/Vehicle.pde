class Vehicle{
  ...
  void update(){
    ...
    stroke(255, 0, 0);
    line(acceleration.x*50+location.x, acceleration.y*50+location.y, location.x, location.y);
    acceleration.mult(0);
  }
  ...
  void separate(ArrayList<Vehicle> vehicles){
    ...
    for(Vehicle other: vehicles){
      ...
      if((d >desiredseparation*1.5) && (d < desiredseparation*2)){
    ...
    if(count > 0 ){
      stroke(0);
            line(sum.x+location.x, sum.y+location.y, location.x, location.y);
      sum.div(count);
